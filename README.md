...
1. 點右上角 **Code → Download ZIP**，下載後解壓縮

2. **先加上執行權限。** 打開「終端機」（啟動台 → 其他 → 終端機），貼上這一行後按 Enter：

   ```
   chmod +x ~/Downloads/claude-desktop-zhtw-mac-main/*.command
   ```

   如果你解壓到別的位置，把 `chmod +x ` 打完（結尾留一個空格），從 Finder 把解壓出來的資料夾拖進終端機視窗，再補上 `/*.command` 按 Enter。

3. 雙擊 **`安裝繁體中文.command`**

4. 閱讀畫面上的說明，輸入 `y` 確認

5. 等待 1-3 分鐘（重新簽章需要時間）

6. 自行開啟 Claude，首次開啟時輸入一次 Mac 登入密碼，點選**「永遠允許」**

要移除時，雙擊 **`還原官方版.command`**。

> **為什麼需要第 2 步：** GitHub 產生的 ZIP 不保留檔案的執行權限，少了這一步，雙擊只會看到 `Permission denied`。若你是用 AirDrop、隨身碟或檔案共享直接拿到整個資料夾，權限通常已保留，可以跳過第 2 步。
